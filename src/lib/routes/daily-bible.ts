...
type TBibleType = keyof typeof BIBLE_TYPE_MAPPER;

export const fetchDailyBible = async (
	props: {
		bibleType?: TBibleType;
		date?: string | Date;
	} = {}
) => {
	const [verses, content] = await Promise.all([getDailyBible(props), getDailyBibleContent(props)]);
	return transfer({ verses, content });
};
...
const getDailyBibleContent = async ({
	bibleType = '기본',
	date = new Date()
}: {
	bibleType?: TBibleType;
